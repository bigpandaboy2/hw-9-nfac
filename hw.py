...
def missing_elements(my_list: list) -> list:
    ...
    full_range_set = set(range(my_list[0], my_list[-1] + 1))
    my_list_set = set(my_list)
    missing = sorted(full_range_set - my_list_set)
...
def count_in_range(my_list: list, start: int, end: int) -> int:
    ...
    return sum(start <= x <= end for x in unique_elements)
...
def least_frequent(my_list: list) -> int:
    ...
    least_frequent_elements = [k for k, v in counts.items() if v == min_frequency]
    return least_frequent_elements[0]
